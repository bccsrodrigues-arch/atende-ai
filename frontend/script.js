...
async function testarIA() {
	...
	try {
      ...
      `;
		...
		if (
			erro.message.includes("fetch") ||
			erro.message.includes("Failed to fetch")
		) {
			console.log(
				"API não disponível, usando resposta mockada para demonstração",
			);
			const respostaMock = {
				resposta:
...
async function deletarTreinamento(id) {
	if (
		!confirm("Tem certeza que deseja remover esta regra de conhecimento da IA?")
	)
		return;
